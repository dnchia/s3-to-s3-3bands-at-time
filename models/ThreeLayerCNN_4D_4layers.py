...
class ThreeLayerCNN:
 ...
 #                first_layer_shape=(5, 5, 64), second_layer_shape=(5, 5, 32), third_layer_shape=(5, 5, 20),
 #                fourth_layer_shape=(5, 5, 3), weights_stdev=0.001, beta=0.1, learning_rate=0.0001, save_file=None):
                 first_layer_shape=(5, 5, 32), second_layer_shape=(5, 5, 16), third_layer_shape=(5, 5, 3),
        ...
        #fl_kernel_height, fl_kernel_width, fl_feature_count = fourth_layer_shape
        #fourth_layer, fourth_layer_weights = self._declare_fourth_layer(
        #    third_layer, (fl_kernel_height, fl_kernel_width, tl_feature_count, fl_feature_count))

        # Declare loss function
        #regularization_weights = [first_layer_weights, second_layer_weights, third_layer_weights, fourth_layer_weights]
        regularization_weights = [first_layer_weights, second_layer_weights, third_layer_weights]
        #loss_function = self._declare_loss_function(fourth_layer, norm_target, regularization_weights)
    ...
    def _declare_third_layer(self, second_layer, layer_shape):
        #return self._declare_layer(second_layer, layer_shape, with_relu=True, layer_name='layer_3')
        return self._declare_layer(second_layer, layer_shape, with_relu=False, layer_name='layer_3')

    #def _declare_fourth_layer(self, third_layer, layer_shape):
    #    return self._declare_layer(third_layer, layer_shape, with_relu=False, layer_name='layer_4')
    ...
    def _declare_loss_function(self, network, net_target, regularization_weights):
        with tf.name_scope('loss'):
            error = tf.squared_difference(network, net_target) # Shape ?, 26, 26, 3 ¿¿¿???
            ...
            # Regularization weights para red: 60, 50, 40, 3
            # 0 --> (5, 5, 21, 60)
            # 1 --> (5, 5, 60, 50)
            # 2 --> (5, 5, 50, 40)
            # 3 --> (5, 5, 40, 3)
